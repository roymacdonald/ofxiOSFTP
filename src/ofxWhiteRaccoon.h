...
#pragma once
//*
@interface ofxWhiteRaccoonDelegate : NSObject<WRRequestDelegate> 
{
    NSString * host;
    NSString * user;
    NSString * pass;
...
class ofxWhiteRaccoon {
public:
    ofxWhiteRaccoon(string host, string user ="", string pass ="");
    ~ofxWhiteRaccoon();
...
private:
    ...
    ofxWhiteRaccoonDelegate * whiteRaccoon;
	

};


class ofxWhiteRaccoonEvent : public ofEventArgs{
public:
	ofxWhiteRaccoonEvent(WRRequestTypes type, bool completed =false, bool failed = false):bRequestCompleted(completed), bRequestFailed(failed), requestType(type){}
	bool bRequestCompleted;
	bool bRequestFailed;
    WRRequestTypes requestType;
};

extern ofEvent<ofxWhiteRaccoonEvent>ofxWhiteRaccoonEventDispatcher;
